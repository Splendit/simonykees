...
public class PersistenceManagerTest extends LicenseCommonTest {
	...
	public void encryptDecryptPersistenceModel() {
		// having an instance of a PersistenceManager together with a
		// PersistenceModel...
		PersistenceManager persistenceMng = PersistenceManager.getInstance();
		Instant nowMin30sec = Instant.now()
			.minusSeconds(30);
		Instant nowMin35sec = Instant.now()
			.minusSeconds(35);
		ZonedDateTime demoExpireDate = ZonedDateTime.now()
			.plusDays(4);
		ZonedDateTime expireTimeStamp = ZonedDateTime.now()
			.plusHours(4);
		ZonedDateTime subscriptionExpires = ZonedDateTime.now()
			.plusDays(350);
		PersistenceModel orgModel = new PersistenceModel("test-number", "test-name", true, LicenseType.TRY_AND_BUY,
				nowMin30sec, demoExpireDate, expireTimeStamp, subscriptionExpires, true, nowMin35sec,
		...
		assertEquals("test-name", decModel.getLicenseeName()
			.orElse(""));
		assertEquals("test-number", decModel.getLicenseeNumber()
			.orElse(""));
		assertEquals(demoExpireDate, decModel.getDemoExpirationDate()
			.orElse(null));
		assertEquals(expireTimeStamp, decModel.getExpirationTimeStamp()
			.orElse(null));
		assertEquals(true, decModel.getLastValidationStatus()
			.orElse(false));
		assertEquals(nowMin30sec, decModel.getLastValidationTimestamp()
			.orElse(null));
		assertEquals(LicenseType.TRY_AND_BUY, decModel.getLicenseType()
			.orElse(null));
		assertEquals(subscriptionExpires, decModel.getSubscriptionExpirationDate()
			.orElse(null));
		assertEquals(true, decModel.getSubscriptionStatus()
			.orElse(false));
		assertEquals(nowMin35sec, decModel.getLastSuccessTimestamp()
			.orElse(null));
		assertEquals(LicenseType.NODE_LOCKED, decModel.getLastSuccessLicenseType()
			.orElse(null));
	}
	...
	public void encryptDecryptWithMissingData() {
		...
		// PersistenceModel...
		PersistenceManager persistenceMng = PersistenceManager.getInstance();
		PersistenceModel orgModel = new PersistenceModel("", null, true, null, Instant.now()
			.minusSeconds(30), null,
				ZonedDateTime.now()
					.plusHours(4),
				null, true, null, null, "");
		persistenceMng.setPersistenceModel(orgModel);
	...
	public void validateNodeLocked() {
		...
		ZonedDateTime subscriptionExpires = ZonedDateTime.now()
			.plusDays(350);
		PersistenceModel orgModel = new PersistenceModel("test-number", "test-name", true, // last
																							// validation
																							// status
				LicenseType.NODE_LOCKED, // license type
				nowMin30sec, // validation time stamp
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
	...
	public void validateFloating() {
		...
		ZonedDateTime subscriptionExpires = ZonedDateTime.now()
			.plusDays(350);
		PersistenceModel orgModel = new PersistenceModel("test-number", "floating test-name", true, // last
																									// validation
																									// status
				...
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
				nowMin35sec, LicenseType.FLOATING, "");
		persistenceMng.setPersistenceModel(orgModel);
		persistenceMng.persist();
	...
	public void validateTryAndBuy() {
		...
		PersistenceModel orgModel = new PersistenceModel("test-number", "floating test-name", true, // last
				...
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
				nowMin35sec, LicenseType.TRY_AND_BUY, "");
		persistenceMng.setPersistenceModel(orgModel);
		persistenceMng.persist();
	...
	public void validateTryAndBuyExiredDemo() {
		...
		Instant nowMin1300sec = Instant.now()
			.minusSeconds(1300);
		Instant nowMin1350sec = Instant.now()
			.minusSeconds(1350);
		ZonedDateTime demoExpireDate = ZonedDateTime.now();
		...
		PersistenceModel orgModel = new PersistenceModel("test-number", "floating test-name", true, // last
				...
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
				nowMin1350sec, LicenseType.TRY_AND_BUY, "");
		persistenceMng.setPersistenceModel(orgModel);
		persistenceMng.persist();
	...
	public void validateExpiredValidationTimestamp() {
		// having stored a PersistedModel with an expired validation
		// time-stamp...
		PersistenceManager persistenceMng = PersistenceManager.getInstance();
		Instant nowMin3601sec = Instant.now()
			.minusSeconds(3601);
		Instant nowMin3605sec = Instant.now()
			.minusSeconds(3605);
		ZonedDateTime demoExpireDate = ZonedDateTime.now()
			.plusDays(4);
		ZonedDateTime subscriptionExpires = ZonedDateTime.now()
		...
		PersistenceModel orgModel = new PersistenceModel("test-number", "test-name", true, // last
				...
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
				nowMin3605sec, LicenseType.NODE_LOCKED, "");
		persistenceMng.setPersistenceModel(orgModel);
		persistenceMng.persist();
	...
	public void validateExpiredSubscription() {
		...
		PersistenceManager persistenceMng = PersistenceManager.getInstance();
		Instant nowMin1000sec = Instant.now()
			.minusSeconds(1000);
		Instant nowMin1005sec = Instant.now()
			.minusSeconds(1005);
		ZonedDateTime demoExpireDate = ZonedDateTime.now()
			.plusDays(4);
		ZonedDateTime subscriptionExpires = ZonedDateTime.now();
		PersistenceModel orgModel = new PersistenceModel("test-number", "test-name", true, // last
				...
				demoExpireDate, null, // expiration time stamp
				subscriptionExpires, true, // last subscription status
				nowMin1005sec, LicenseType.NODE_LOCKED, "");
		persistenceMng.setPersistenceModel(orgModel);
		persistenceMng.persist();
