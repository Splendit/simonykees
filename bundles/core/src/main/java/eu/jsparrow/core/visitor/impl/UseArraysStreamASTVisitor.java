...
/**
 * A visitor for transforming {@code Arrays.asList(T..values).stream()} into either of 
 * the following: 
 * 
 * <ul>
 * 	<li>
 * 	Unboxed specialized stream ({@link IntStream}, {@link LongStream}, or {@link DoubleStream}).
 * 	For example, the following stream: 
 * 
 *  <pre>{@code Arrays.asList(1, 2, 3).stream().forEach(val -> {})}</pre>
 *  is transformed to: 
 *  <pre>{@code Arrays.stream(new int[]{1, 2, 3}).forEach(val -> {})}</pre>
 *  Thus, avoiding an unnecessary boxing of the elements of the stream. 
 * 	</li>
 * 
 * 	<li>
 * 	Stream generated by the short hand method {@link Stream#of(Object...)}. For example, 
 * 	the following code: 
 * 	<pre>{@code Arrays.asList("1", "2", "3").stream().forEach(val -> {})}</pre>
 * 	is transformed to:
 * 	<pre>{@code Stream.of("1", "2", "3").forEach(val -> {})}</pre>
 * 	</li>
 * </ul>
 * 
 * @since 3.18.0
