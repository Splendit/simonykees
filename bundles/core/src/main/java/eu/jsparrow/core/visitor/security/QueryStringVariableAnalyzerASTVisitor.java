 ...
 *
 */
public class QueryStringVariableAnalyzerASTVisitor extends ASTVisitor {
	...
	private List<Block> blockAncestors;

	public QueryStringVariableAnalyzerASTVisitor(SimpleName variableName, ASTNode declaration,
	...
	public boolean visit(VariableDeclarationFragment fragment) {
		if (this.declarationFragment == fragment) {
			blockAncestors = createBlockAncestorList(simpleNameAtUsage, fragment);
			if (blockAncestors.isEmpty()) {
				unsafe = true;
				return false;
			}
		...
		if (structuralDescriptor == Assignment.LEFT_HAND_SIDE_PROPERTY) {
			...
			if (assignment.getOperator() == Assignment.Operator.PLUS_ASSIGN && initializer != null) {
				...
				if (rightHandSide.getNodeType() != ASTNode.NULL_LITERAL) {
					// TODO:
					// if the block surrounding the initializer is found in
					// blockAncestors, then the following operations are carried
					// out
					initializer = rightHandSide;
					componentStore.storeComponents(initializer);
					// TODO:
					// if the block surrounding the initializer is NOT found in
					// blockAncestors, then unsafe is set to true.
				}
			} else {
				unsafe = true;
			}
	...
	Block getBlockSurroundingSimpleNameAtUsage(SimpleName simpleNameAtUsage) {
		...
		if (simpleNameAtUsage.getParent()
			.getLocationInParent() != ExpressionStatement.EXPRESSION_PROPERTY) {
			return null;
		}
		// get the surrounding expression statement for simpleNameAtUsage
		ExpressionStatement expressionStatement = ASTNodeUtil.getSpecificAncestor(simpleNameAtUsage,
				ExpressionStatement.class);
		// return the surrounding block of the expression statement
		return ASTNodeUtil.getSpecificAncestor(expressionStatement, Block.class);
