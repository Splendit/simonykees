    ...
    if (isLiveEnvironment) {
        stage(stageName) {
          ...
          // tag-deployment.sh uses the same mechanism
    		def buildNumber = sh(returnStdout: true, script: "pcregrep -o1 \"name='eu.jsparrow\\.feature\\.feature\\.group' range='\\[.*,((\\d*\\.){3}\\d{8}-\\d{4})\" releng/eu.jsparrow.site.neon/target/p2content.xml").trim()
