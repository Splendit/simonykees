 ...
 * </ul>
 * <p>
 * Furthermore, the members of the same type (except for fields and methods),
 * are also sorted according to their modifier. The priority of the modifiers is
 * as follows:
 * <ul>
 * <li>static</li>
 * <li>public</li>
...
public class RearrangeClassMembersASTVisitor extends AbstractASTRewriteASTVisitor {
	...
	public boolean visit(TypeDeclaration node) {
		...
		if (compilationUnit != null) {
			List<Comment> comments = ASTNodeUtil.returnTypedList(compilationUnit.getCommentList(), Comment.class);

			List<BodyDeclaration> bodyDeclarations = ASTNodeUtil.returnTypedList(node.bodyDeclarations(),
					BodyDeclaration.class);

			Map<BodyDeclaration, List<Comment>> boundedComments = extractBoundedComments(node, comments,
					bodyDeclarations);

			// classify the body declarations according to their type

			// static fields and static initializers are handled together
			List<BodyDeclaration> staticFieldsAndInitializers = applyFilter(bodyDeclarations,
					member -> isStaticMember(member)
							&& (FieldDeclaration.class.isInstance(member) || Initializer.class.isInstance(member)));

			List<BodyDeclaration> instanceFields = applyFilter(bodyDeclarations,
					member -> FieldDeclaration.class.isInstance(member) && !isStaticMember(member));

			List<BodyDeclaration> instanceInitializers = applyFilter(bodyDeclarations,
					member -> Initializer.class.isInstance(member) && !isStaticMember(member));

			List<MethodDeclaration> methodsDeclarations = filterByDeclarationType(bodyDeclarations,
					MethodDeclaration.class);

			List<MethodDeclaration> constructors = methodsDeclarations.stream()
				.filter(MethodDeclaration::isConstructor)
				.collect(Collectors.toList());

			List<MethodDeclaration> methods = methodsDeclarations.stream()
				.filter(method -> !method.isConstructor())
				.collect(Collectors.toList());

			List<EnumDeclaration> enums = filterByDeclarationType(bodyDeclarations, EnumDeclaration.class);

			List<AnnotationTypeDeclaration> annotations = filterByDeclarationType(bodyDeclarations,
					AnnotationTypeDeclaration.class);

			List<AnnotationTypeMemberDeclaration> annotationMembers = filterByDeclarationType(bodyDeclarations,
			...
			/**
			 * Fields are not sorted by the access modifier because they may
			 * reference each-other during initialization.
			 */
			...
			if (!sortedDeclarations.isEmpty() && startFrom >= 0) {
				...
				firstDeclComments.forEach(comment -> comment.getAlternateRoot()
					.delete());
				...
				for (int i = startFrom + 1; i < sortedDeclarations.size(); i++) {
					...
					declComments.forEach(comment -> comment.getAlternateRoot()
						.delete());
	...
	/**
	 * Sorts the given list of body declarations of the same type, by the
	 * modifier. Static members have higher priority. Then, comes public,
	 * protected, package protected and private.
	 * 
	 * @param members
	 *            body declarations to be sorted
	...
	private <T extends BodyDeclaration> List<T> sortMembers(List<T> members) {
		...
		List<T> staticMembers = members.stream()
			.filter(this::isStaticMember)
			.collect(Collectors.toList());
		// extract instance members
		List<T> instanceMembers = members.stream()
	...
	/**
	 * Checks whether the given body declaration is a static or an instance
	 * declaration.
	 * 
	 * @param member
	 *            a body declaration
	...
	/**
	 * Sorts the given list of body declarations according to their access
	 * modifiers. The priority of the modifiers is as follows:
	 * <ul>
	 * <li>public</li>
	 * <li>protected</li>
	 * <li>(none)</li>
	 ...
	 * 
	 * @param member
	 *            body declarations of the same type, to be sorted.
	 * @return sorted list of body declarations
	 */
	...
	/**
	 * Extracts the body declarations of the given from the given list of the
	 * body declarations.
	 * 
	 * @param bodyDeclarations
	 *            list of original body declarations
	 * @param type
	 *            type of the declarations to be filtered.
	 * @return
	 */
	private <T extends BodyDeclaration> List<T> filterByDeclarationType(List<BodyDeclaration> bodyDeclarations,
			Class<T> type) {
	...
	/**
	 * Extracts the body declarations having the given modifier flag from the
	 * given list of body declarations.
	 * 
	 ...
	 */
	private <T extends BodyDeclaration> List<T> filterByModifier(List<T> members,
			Predicate<Modifier> modifierPredicate) {
		return applyFilter(members, member -> ASTNodeUtil.hasModifier(member.modifiers(), modifierPredicate));
	}

	/**
	 * Extracts the body declarations with <b>NO</b> access modifier. The
	 * difference from {@link #filterByModifier(List, int)} is that it checks
	 * for the absence of an access modifier.
	 * 
	 * @param members
	 *            list of body declarations.
	...
	private <T extends BodyDeclaration> List<T> filterByPackageProtectedModifier(List<T> members) {
		Predicate<T> packageProtectedFilter = member -> {
			...
			return !Modifier.isProtected(flag) && !Modifier.isPublic(flag) && !Modifier.isPrivate(flag);
		};
	 ...
	 *            list to filter
	 * @param filter
	 *            defines the filter for the collection.
	...
	/**
	 * Finds the comments that belong to a member of the class. A comment is
	 * considered to belong to a member of the class if it is placed above it,
	 * with possible empty lines in between.
	 * 
	 * @param node
	 *            represents the body of the whole class.
	 * @param comments
	 *            list of all comments in the compilation unit.
	 * @param bodyDeclarations
	 *            list of body declarations in the class.
	 * 
	 * @return a map representing the comments belonging to a member of the
	 *         class.
	 */
	private Map<BodyDeclaration, List<Comment>> extractBoundedComments(TypeDeclaration node, List<Comment> comments,
		...
		if (!bodyDeclarations.isEmpty()) {
			...
			for (int i = 1; i < bodyDeclarations.size(); i++) {
				...
				while (j < comments.size() && comments.get(j)
					.getStartPosition() < decl.getStartPosition()) {
	...
	/**
	 * Finds the comments belonging to the very first declaration in the body of
	 * the class.
	 * 
	 * @param node
	 *            represents the body of the whole class.
	...
	private List<Comment> extractFirstDeclComments(TypeDeclaration node, List<BodyDeclaration> bodyDeclarations,
		...
		if (!bodyDeclarations.isEmpty()) {
			...
			int declStartPos = bodyDeclarations.get(0)
				.getStartPosition();
	...
	/**
	 * Checks whether a given comment is placed between the given two
	 * consecutive body declarations.
	 * 
	 * @param comment
	...
	private boolean fallsBetween(Comment comment, BodyDeclaration previousDeclaration, BodyDeclaration declaration) {
		...
		return commentStartPos > previousEndPos && commentEndPos < startPos;
	}
}
