...
/**
 * Renames the private fields (except for static final ones) to comply with the
 * naming convention: "^[a-z][a-zA-Z0-9]*$" i.e. a lower case prefix followed by
 * any sequence of alpha-numeric characters. For example, the fields of the the
 * following class:
 * 
 * <pre>
 * 
 ...
 * </pre>
 * 
 * Note that the field {@code _int} is not renamed because the resulting new
 * name would be {@code int} which is not eligible for a variable name.
 * 
 * @author Ardit Ymeri
 * @since 1.2
 *
	...
	public boolean visit(TypeDeclaration typeDeclaration) {
		...
		declaredFieldNames = fields.stream()
			.flatMap(fieldDecl -> ASTNodeUtil
				.convertToTypedList(fieldDecl.fragments(), VariableDeclarationFragment.class)
				.stream())
								...
								generateNewIdetifier(fragmentIdentifier)
									...
									.ifPresent(newName -> {
										...
										if (typeParent != null
											 ...
											 */
											collidingWithOuterTypeField = NamingConventionUtil
												.hasField(typeDeeclarationParent, fragmentName);
