 ...
 *
 */
public class WhileToForEachASTVisitor extends AbstractAddImportASTVisitor {
				...
				if (ASTNode.METHOD_INVOCATION == rhs.getNodeType()) {
					...
					if (conditionExpression != null && Expression.SIMPLE_NAME == conditionExpression.getNodeType()) {
						...
						if (ClassRelationUtil.isInheritingContentOfTypes(iterableTypeBinding,
							...
							WhileLoopIteratingIndexASTVisitor indexVisitor = new WhileLoopOverListsASTVisitor(index,
					...
					if (LENGTH.equals(name.getIdentifier()) && qualifier.isSimpleName()) {
						...
						if (iterableTypeBinding != null && iterableTypeBinding.isArray()) {
							...
							Block outerBlock = ASTNodeUtil.getSpecificAncestor(node, Block.class);
							WhileLoopIteratingIndexASTVisitor indexVisitor = new WhileLoopOverArraysASTVisitor(index,
									iterableNode, node, outerBlock);
							outerBlock.accept(indexVisitor);
	...
	private void replaceWithEnhancedFor(WhileStatement node, SimpleName iterableNode,
			WhileLoopIteratingIndexASTVisitor indexVisitor, Type iteratorType) {
		/*
	...
	private void storeTempName(WhileStatement node, String newIteratorIdentifier) {
	...
	private String generateTempIteratorKey(WhileStatement node) {
