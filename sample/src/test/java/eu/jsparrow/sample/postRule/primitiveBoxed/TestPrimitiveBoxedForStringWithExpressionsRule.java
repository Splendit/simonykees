package eu.jsparrow.sample.postRule.primitiveBoxed;
...
public class TestPrimitiveBoxedForStringWithExpressionsRule {
	...
	public String testIntegerValueOfCascadeExpressions(int input) {
		return Integer.toString(input + intSampleMethod()) 
				+ Integer.toString(input + "abc".length());
	}
	
	public String testIntegerValueOfInIfCondition(int input) {
		String result = "";
		if(!Integer.toString(input).isEmpty()
				|| Integer.toString(5).length() == 1) {
	...
	public String testMethodInvocationIntegerBoxing(int input) {
		String val = Integer
				.toString(intSampleMethod(Integer.toString(3)) + 4);
		return val;
	}
