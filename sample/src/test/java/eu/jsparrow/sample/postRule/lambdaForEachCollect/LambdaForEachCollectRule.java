package eu.jsparrow.sample.postRule.lambdaForEachCollect;
...
.addAll(objectList.stream().map(o -> o.substring(0)) // comment after map
.collect(Collectors.toList()));
	...
	public String convertForEachExpressionToCollect(String input) {
		...
		oStrings.addAll(objectList.stream().map(o -> o.toString()).collect(Collectors.toList()));
	...
	public String referencingOtherLocalVars(String input) {
		...
		List<String> objectList = new ArrayList<>();
		String s = "";
		oStrings.addAll(objectList.stream().filter(oString -> oString.equals(s)).map(o -> o.substring(0)).collect(Collectors.toList()));
	...
	public String addingToFieldMethod(String input) {
		List<String> objectList = new ArrayList<>();
		String s = "";
		listField.addAll(objectList.stream().filter(oString -> oString.equals(s)).map(o -> o.substring(0)).collect(Collectors.toList()));
