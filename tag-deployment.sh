...
# building the tag
tag=`pcregrep -o1 "name='eu\.jsparrow\.feature\.feature\.group' range='\[.*,(.*-\d{4})" releng/eu.jsparrow.site.oxygen/target/p2content.xml`
echo tag=$tag
suffix=`pcregrep -o1 "name='(\w+\.\w+\.\w+)\.feature\.group' range='\[.*,(.*-\d{4})" releng/eu.jsparrow.site.oxygen/target/p2content.xml`
echo suffix=$suffix
