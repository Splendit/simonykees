...
import at.splendit.simonykees.license.netlicensing.model.NodeLockedModel;
import at.splendit.simonykees.license.netlicensing.model.PersistenceModel;
import at.splendit.simonykees.license.netlicensing.model.TryAndBuyModel;
...
public class LicenseValidatorTest extends LicenseCommonTest {
	...
	public void validateNodeLockedLicense() throws InterruptedException {
		...
		LicenseeModel licensee = new LicenseeModel(NODE_LOCKED_LICENSEE_NAME, NODE_LOCKED_LICENSEE_NUMBER, nodeLocked,
				productNumber);
		...
		// expecting the validation result to be cached and validation status to
		// be true
	...
	public void validateNodeLockedHwIdFailureStatus() throws InterruptedException {
		...
		LicenseValidator.doValidate(licensee);// sending a validate request with
												// incorrect HW ID
		Thread.sleep(WAIT_FOR_VALIDATION_RESPONSE_TIME);
		...
		assertFalse(checker.isValid()); // validation should be false
		assertEquals(LicenseType.NODE_LOCKED, checker.getType());
		assertEquals(LicenseStatus.NODE_LOCKED_HW_ID_FAILURE, checker.getLicenseStatus()); // hw
																							// id
																							// failure
																							// shall
																							// be
	...
	public void checkExistingLicensee() {
		...
		boolean existingLicensee = LicenseValidator.isValidLicensee(NODE_LOCKED_LICENSEE_NUMBER);
		assertTrue(existingLicensee);
		boolean nonExistingLicensee = LicenseValidator.isValidLicensee("non-existing-licensee-number");
